...
DeepSeek 官方价卡与峰谷时段由 `deepseek_official_pricing.go` 自动同步，不再只依赖硬编码常量。官方未提供价格查询 API（`/v1/models` 仅返回模型名、`/user/balance` 仅返回余额），数据源为官方文档中文定价页（`pricing.deepseek_pricing_url`，人民币基准价；英文页解析能力保留但默认不作为数据源），按 `pricing.deepseek_sync_interval_hours`（默认 6 小时）抓取，解析各模型空闲时段单价、高峰倍率以及峰谷时段（含时区与是否仅工作日）。解析结果须通过自检（两个模型族齐全、缓存命中价不得高于未命中价、峰谷倍率与单价在合理区间、时段区间合法、时区与币种非空），通过后原子替换内存快照并落盘 `{pricing.data_dir}/deepseek_official_pricing.json`；抓取或校验失败保留上一份成功快照，快照中非当前来源产出的币种会被清理。**站点口径以人民币价为基准**：CNY 站点原样使用；USD 站点按 `settings.usd_exchange_rate`（1 USD = N CNY）折算 —— 与同步脚本折算组7/8、渠道价的方式完全一致，避免同一模型出现两套美元数字；其它币种、或 USD 站点未配置汇率时不覆盖价卡（保持上游目录价）并打一条去重告警日志便于排查；内置常量仅在站点币种为人民币时兜底。站点口径缓存在设置写入后立即失效，切换币种或改汇率最多 60 秒生效。数据源 URL 受 `security.url_allowlist.pricing_hosts` 约束；响应为 JSON 时按等价结构解析（币种取 `currency` 字段），便于官方后续提供 API 时无需改代码即可切换。该快照只作用于「默认价卡」路径：分组或渠道显式定价仍然优先，且不叠加官方峰谷倍率。峰谷倍率与时段是比值与时刻，与币种无关；无快照时回退内置规则（工作日 UTC 01:00-04:00 与 06:00-10:00）。
