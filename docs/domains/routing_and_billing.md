...
DeepSeek 官方价卡与峰谷时段由 `deepseek_official_pricing.go` 自动同步，不再只依赖硬编码常量。官方未提供价格查询 API（`/v1/models` 仅返回模型名、`/user/balance` 仅返回余额），数据源为官方文档定价页（默认中文页，人民币原生价格），按 `pricing.deepseek_sync_interval_hours`（默认 6 小时）抓取，解析各模型空闲时段单价、高峰倍率以及峰谷时段（含时区与是否仅工作日）。解析结果须通过自检（两个模型族齐全、缓存命中价不得高于未命中价、峰谷倍率与单价在合理区间、时段区间合法、时区与币种非空），通过后原子替换内存快照并落盘 `{pricing.data_dir}/deepseek_official_pricing.json`；抓取或校验失败保留上一份成功快照，最终兜底为内置常量。数据源 URL 受 `security.url_allowlist.pricing_hosts` 约束；响应为 JSON 时按等价结构解析，便于官方后续提供 API 时无需改代码即可切换。该快照只作用于「默认价卡」路径：分组或渠道显式定价仍然优先，且不叠加官方峰谷倍率。峰谷判定使用快照中的时区与工作日规则；无快照时回退内置规则（工作日 UTC 01:00-04:00 与 06:00-10:00）。
