...
- [上游上下文元数据透传](#model_catalog_upstream_context)：修改 `/v1/models` 的上下文透传、缓存或刷新策略时读取。
...
`/v1/models` 的每个条目可以附带上游声明的上下文信息：`context_length`（上下文窗口）和 `max_completion_tokens`（单次最大输出）。这类字段是**透传**而非推断：

- 来源限于同一账号的 `GET /v1/models` 响应，仅对 API-Key 型账号（OpenAI 平台或国内供应商）尝试；上游返回的 `context_length`/`context_window`/`max_input_tokens` 与 `max_completion_tokens`/`max_output_tokens` 按优先级取第一个正值。
- 快照按账号缓存（TTL 6 小时，持久化在账号 `extra.upstream_models_meta`），刷新在后台异步执行并有单飞保护，失败保留 5 分钟退避；刷新永远不阻塞 `GET /v1/models` 的响应。
- **只有上游明确给出正数时才下发该字段**。上游未提供、只给了非正数、快照尚未就绪或刷新失败时，响应与历史结构逐字节一致（字段省略），不按内置目录、模型名或价格阶梯合成上下文值。
- OpenAI 兼容、Anthropic 兼容、复合 Key 三种响应形态共用同一份元数据与省略规则；复合 Key 的元数据键与带分组前缀的模型 ID 对齐，避免前缀模型查不到上下文。
...
- 上游上下文元数据是可选的描述信息，只能来自上游 `/v1/models` 的原始声明；网关不得用内置目录、模型家族或价格阶梯补齐缺失值，也不得让它影响可请求模型集合。
