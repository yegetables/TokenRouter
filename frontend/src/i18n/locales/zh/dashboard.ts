...
// Dashboard
  dashboard: {
    ...
    actualUnitHint: '用户实扣费用，按本站余额单位显示。数值已含分组倍率与用户专属倍率。',
    standardUnitHint: 'USD 基准价：按模型或渠道的基础定价计算，不含任何倍率。符号固定为 $，用于与实扣费用区分口径。',
    accountUnitHint: 'USD 基准价：账号口径成本，用于账号配额统计与成本分析，不影响用户实扣费用。',
    input: '输入',
    output: '输出',
    cache: '缓存',
...
// Usage
  usage: {
    ...
    modelUsageCache: {
      ...
      quota: '额度',
      notApplicable: '不适用',
      creationNotReported: '该上游不回报缓存写入 Token，此处仅显示缓存读取',
      cacheRateHint: '缓存命中率 = 缓存读取 ÷（输入 + 缓存创建 + 缓存读取）。口径为上游提示词缓存，不含网关级响应缓存。',
      aggregateHint: '全部模型按 Token 加权：Σ缓存读取 ÷ Σ输入侧总量，不是各模型命中率的算术平均。',
    },
    totalCost: '总消费',
    standardCost: '标准',
    actualCost: '实际',
