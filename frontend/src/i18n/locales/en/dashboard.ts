...
// Usage
  usage: {
    ...
    modelUsageCache: {
      ...
      quota: 'Quota',
      notApplicable: 'N/A',
      creationNotReported: 'This upstream does not report cache write tokens; showing cache read only',
      cacheRateHint: 'Cache hit rate = cache read ÷ (input + cache creation + cache read). This covers upstream prompt caching, not gateway-level response cache.',
      aggregateHint: 'Weighted across all models by tokens: Σcache read ÷ Σinput-side total, not an average of per-model rates.',
    },
    ...
    standardDescription: 'The standard billed cost. Calculated from the current model or channel base pricing, and is not affected by group multipliers, user-specific multipliers, or account billing multipliers.',
    actualUnitHint: 'Actually charged to the user, shown in this site’s balance unit. Already includes group and user-specific multipliers.',
    standardUnitHint: 'USD reference price: model or channel base pricing, with no multipliers applied. The $ symbol marks a different basis from the actual charge.',
    accountUnitHint: 'USD reference price: account-side cost used for account quota statistics and cost analysis; does not affect what the user is charged.',
